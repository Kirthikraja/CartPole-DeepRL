...
update_ratios = [2, 4, 8]  #  Small, Medium, High
...
def train_q_learning(update_ratio, num_runs=4):
    num_envs = 8  
    ...
    for run in range(num_runs):
        print(f"\n Training Run {run + 1}/4 for Update Ratio: {update_ratio}")
...
for ratio in update_ratios:
    results = train_q_learning(ratio)
    ...
    labels.append(f"Update Ratio: {ratio}")
...
plt.title("Ablation Study: Update-to-Data Ratio Comparison")
plt.legend()
plt.grid()
plt.show()
