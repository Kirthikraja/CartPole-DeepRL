...
class QNetwork(nn.Module):
    def __init__(self, state_size, action_size, network_size):
        ...
        layers.append(nn.Linear(input_dim, action_size))  # Output Layer
...
def train_q_learning(network_size, num_runs=4):
    ...
    for run in range(num_runs):
        print(f"\n Training Run {run + 1}/4 for Network Size: {network_size}")
...
for network_size in network_sizes:
    results = train_q_learning(network_size)
...
plt.title("Ablation Study: Network Size Comparison")
plt.legend()
plt.grid()
plt.show()
