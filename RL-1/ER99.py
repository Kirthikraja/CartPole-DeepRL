...
def update_q_network(q_network, optimizer, replay_buffer, device):
    if replay_buffer.size() < batch_size:
        return  # Don't train until buffer is filled
...
for run in range(num_runs):
    print(f"\n🔄 Training Run {run + 1}/{num_runs} with Experience Replay")
    ...
    q_network = QNetwork(state_size, action_size, network_size).to(device)
    ...
    while total_steps < max_steps:
        ...
        while not np.all(done_flags):  
            ...
            for _ in range(update_to_data_ratio):
                update_q_network(q_network, optimizer, replay_buffer, device)
...
plt.title("5 Repetitions of Q-Learning with Experience Replay on CartPole")
plt.legend()
plt.show()
...
plt.xlabel("Episodes")
plt.ylabel("Total Reward per Episode")
plt.title("Experience Replay on CartPole")
plt.legend()
plt.grid()
plt.show()
...
np.save("ER99.npy", smoothed_mean_returns)
