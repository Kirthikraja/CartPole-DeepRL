...
def train_q_learning(learning_rate, num_runs=4):
    ...
    for run in range(num_runs):
        print(f"\n Training Run {run + 1}/4 for Learning Rate: {learning_rate}")
...
learning_rates = [0.00001, 0.0000501, 0.0005]  #  Small, Medium, High
all_results = []
labels = []

for lr in learning_rates:
    results = train_q_learning(lr)
...
plt.title("Ablation Study: Learning Rate Comparison")
plt.legend()
plt.grid()
plt.show()
