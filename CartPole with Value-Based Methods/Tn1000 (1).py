...
def update_q_network(states, actions, rewards, next_states, dones, q_network, target_network, optimizer, device):
...
def train_q_learning(num_runs=4):
    ...
    for run in range(num_runs):
        print(f"\n Training Run {run + 1}/{num_runs} with Target Network")
        ...
        while total_steps < max_steps:
            ...
            while not np.all(done_flags):  
                ...
                for _ in range(update_to_data_ratio):
                    update_q_network(states, actions, rewards, next_states, dones, q_network, target_network, optimizer, device)
...
plt.title("Target Network: Q-Learning on CartPole")
plt.legend()
plt.grid()
plt.show()
...
np.save("Tn100.npy", smoothed_mean_returns)
